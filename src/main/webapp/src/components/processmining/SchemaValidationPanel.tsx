...
const SchemaValidationPanel: React.FC<SchemaValidationPanelProps> = ({
    ...
    return (
      <div key={ft} className="border border-primary/20 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2.5 bg-primary/5 border-b border-primary/10">
          <span className="text-sm font-semibold text-slate-200">{FIELD_LABELS[ft]}</span>
          {entry && confidenceBadge(entry.confidence)}
        </div>
        <div className="divide-y divide-primary/10">
          {topics.map(topic => (
            <div key={topic} className="flex items-center gap-3 px-4 py-2.5">
              <span className="text-xs font-mono text-slate-400 w-40 truncate" title={topic}>
                {topic}
              </span>
              ...
              <input
                ...
                className="flex-1 bg-background-dark border border-primary/20 rounded-lg px-2.5 py-1.5 text-xs font-mono text-slate-100 focus:border-primary/50 outline-none"
              />
            </div>
          ))}
        </div>
        {entry.conflicts && entry.conflicts.length > 0 && (
          <div className="px-4 py-2 bg-amber-500/10 border-t border-amber-500/20">
            <p className="text-xs text-amber-400 font-medium">Conflicts detected:</p>
            {entry.conflicts.map((c, i) => (
              <p key={i} className="text-xs text-amber-400/80 mt-0.5">{c}</p>
            ))}
          </div>
        )}
      </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-slate-100 mb-1">Validate Schema Mapping</h2>
        ...
        <div>
          ...
          <div className="grid gap-2">
            {result.topics.map(tp => (
              ...
              >
                <span className="text-xs font-mono text-slate-300">{tp.name}</span>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-500">{tp.format}</span>
                  <span className="text-xs text-slate-500">{tp.fields?.length ?? 0} fields</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex gap-3 pt-2">
        <button
          ...
          disabled={loading}
          className="flex-1 flex items-center justify-center gap-2 py-3 px-6 bg-primary text-white rounded-xl font-semibold text-sm hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {loading ? (
            <>
