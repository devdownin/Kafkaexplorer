...
const QueryWorkbench: React.FC = () => {
  ...
  const applyWindowLogic = () => {
    ...
    let template = "";
    if (windowType.includes("Tumbling")) {
      template = `-- Tumbling Window: Grouping by ${windowSize} ${unit}\nSELECT\n  window_start, window_end, COUNT(*) as cnt\nFROM (\n  SELECT * FROM source_table\n)\nGROUP BY\n  window_start, window_end,\n  TUMBLE(TABLE source_table, DESCRIPTOR(event_time), INTERVAL '${windowSize}' ${unit});`;
    } else if (windowType.includes("Hopping")) {
      template = `-- Hopping Window: Size ${windowSize * 2} ${unit}, Slide ${windowSize} ${unit}\nSELECT\n  window_start, window_end, COUNT(*) as cnt\nFROM (\n  SELECT * FROM source_table\n)\nGROUP BY\n  window_start, window_end,\n  HOP(TABLE source_table, DESCRIPTOR(event_time), INTERVAL '${windowSize}' ${unit}, INTERVAL '${windowSize * 2}' ${unit});`;
    } else {
       template = `-- Session Window: Gap ${windowSize} ${unit}\nSELECT\n  window_start, window_end, COUNT(*) as cnt\nFROM (\n  SELECT * FROM source_table\n)\nGROUP BY\n  window_start, window_end,\n  SESSION(TABLE source_table, DESCRIPTOR(event_time), INTERVAL '${windowSize}' ${unit});`;
    }
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 flex flex-col min-h-0 relative">
          ...
          <div className="h-1/2 border-t border-primary/20 flex flex-col bg-background-dark/60 relative">
            {/* Statistics Bar */}
                          ...
                          <td key={col} className="px-4 py-3 text-xs font-mono text-slate-300">
                            {typeof row[col] === 'object' ? (
                              <div className="flex gap-2 items-center">
                                <span className="truncate max-w-[200px]">{JSON.stringify(row[col])}</span>
                                <button
                                  onClick={() => {
                                    const json = row[col];
                                    const select = Object.keys(json).map(k => `  JSON_VALUE(payload, '$.${k}') as ${k}`).join(',\n');
                                    setSql(`SELECT\n${select}\nFROM source_table`);
                                  }}
                                  className="opacity-0 group-hover:opacity-100 p-1 bg-primary/20 rounded text-[8px] text-primary uppercase font-bold"
                                >
                                  Select Fields
                                </button>
                              </div>
