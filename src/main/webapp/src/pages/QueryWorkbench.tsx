...
import {
  ...
  planRun, detectStatementType, previewStatement, isJobModeStatement,
...
const QueryWorkbench: React.FC = () => {
  ...
  const executeStatement = async (sqlToRun: string): Promise<StatementOutcome> => {
    ...
    /*
     * L'éditeur lit : il rend des lignes. Un INSERT est donc refusé ici, **avec sa cause**, plutôt
     * que d'être envoyé au moteur pour se voir répondre par la whitelist — « Only SELECT, EXPLAIN,
     * SHOW, DESCRIBE and CREATE TABLE statements are allowed » se lit comme une restriction de
     * sécurité, là où ce qui est en jeu est la forme de cet écran. Il y avait ici deux gardes qui
     * aiguillaient entre lecture et mode Job ; ce mode a été retiré (il ne fonctionnait pas), et
     * elles ont survécu à ce qu'elles arbitraient, en référençant un `executionMode` qui n'existe
     * plus. Même règle que partout sur cette page : un refus qui nomme la cause vaut mieux qu'un
     * refus qui nomme une règle.
     */
    if (isJobModeStatement(statementType)) {
      return refuse({
        title: 'INSERT INTO is not run by the SQL editor',
        hint: 'This editor executes reads and returns rows. Run the pipeline as a Flink job of your own; the editor reads what it produces.',
        raw: `The SQL editor does not submit ${statementType} statements.`,
      });
    }
   ...
   *
   * Il pose une lecture bornée, et rien d'autre. Il suivait le mode d'exécution : en mode Job il
   * posait un squelette d'`INSERT INTO`, cible choisie dans le catalogue et nom présélectionné.
   * Ce mode a été retiré parce qu'il ne fonctionnait pas, et un geste dont la seule issue possible
   * est un échec vaut moins que pas de geste — les aides qui n'existaient que pour lui
   * (`pickSinkTable`, `sinkNameRange`) sont parties avec, ne laissant ici que leurs appels.
   */
