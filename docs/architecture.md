...
- **The MCP Scope Guard Reads The Statement, Not Just The Argument**: `explorer.mcp.allowed-topic-prefixes` is applied to `kex_sql_query` by resolving every source in the SQL back to the topic it would register, because a prefix is written in topic terms while a statement names Flink tables — where a dot has already become an underscore. A reference that matches no topic is refused rather than read: a hand-written table carries its own `topic` option, which is the one way left out of the scope once resolution is in place. Without it the setting stopped `kex_preview_messages` on a topic and one `SELECT` read it. See [notes/mcp-server.md](notes/mcp-server.md).
- **The MCP Endpoint Authenticates, The Console Does Not**: `/mcp` and every state-changing `/api/mcp/**` call require a bearer token over TLS (`explorer.mcp.auth-token`, `explorer.mcp.require-tls`), fail-closed — no token configured is a 503, not an open endpoint — and the credential is hashed into the identity that quarantine, the rate limit and the audit trail are keyed on, so a reconnect cannot reset a lever. The console's own reads stay unauthenticated like the rest of this application, which authenticates nobody (`SECURITY.md`); its switches take the token the operator pastes into the page. This is an interim for the OAuth 2.1 `SPEC-MCP.md` specifies, and says so.
