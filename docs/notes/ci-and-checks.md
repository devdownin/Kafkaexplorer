...
split is the design: the trace — which tools were called, in what order, and what happened after a
refusal — is captured by the harness's own MCP client and asserted mechanically, with no model and
no prose in the loop; only what the agent *asserts* is judged, by a second model call, on a grid of
decisions (`mustAssert` / `mustNotAssert` / `mustQualify` / `mustCite`) rather than of wordings. The
grid's rule is this repository's own precedent from `LlmAnalysisEvalTest`: a tighter assertion on a
model's prose is a test that fails on a paraphrase. `mustQualify` and `mustCite` are what catch the
right answer for the wrong reason — an agent that answers correctly without citing the coverage it
was served did not read the coverage, it was lucky.

**Everything deterministic lands first, and runs in `mvn verify`.** `AgentScenario`,
`ScenarioLoader`, `ToolCall` and `ToolCallTrace` are the model, the reader and verdict 1, with their
own unit tests and no model call anywhere: `ToolCallTraceTest` builds the traces an agent could
really produce — an ignored resume token, a refused call re-issued with its constraint dropped, a
retry that did not wait as long as the rate limiter asked, a tool name that is not in `tools/list` —
and asserts the sentence the harness prints about each. **The MCP client and the loop are in that
half too**, which is what `AgentModel` exists as an interface for: `McpHttpClient` is a third-party
client by construction — JSON-RPC over HTTP, answers read as text, never a bean and never one of the
server's own SDK types, which is what lets it notice the Jackson 3 transport losing a `Measured` the
Jackson 2 REST surface still serialises — and `AgentRunnerTest` drives a scripted model through it
against a stub server. Three of the loop's rules are asserted there rather than only in a run that
costs money: the call ceiling stops the loop and is *reported* (a run that quietly truncated and then
answered would show a passing trace for an agent that never finished); a refusal goes back to the
model as content rather than being absorbed, since what the agent does after being told no is the
...
never had — would otherwise load, run, and report green having asserted nothing. A scenario file is
the entire content of a case, so a typo in one is not a case that tests slightly less; it is a case
that tests less than its author believes, silently. The same reasoning refuses a duplicated key
(YAML keeps the last one), an `id` that disagrees with its file name (nobody finds it from a failure
message), a `trace.maxCalls` above `maxToolCalls` (a bound that can never bind), a tool named in both
`mustCall` and `mustNotCall`, and a scenario asserting nothing at all. `serverConfig` accepts
...
- **`docs/check-config-yaml.py` pose l'autre moitié de la question** — depuis le YAML plutôt que depuis la prose : pour chaque clé sous `explorer:`, existe-t-il du code qui lit la propriété qu'elle lie ? `check-config-table.py` résout les réglages que la *documentation* nomme, donc il ne peut pas voir un réglage que personne n'a écrit — et c'est précisément là qu'un bouton pourrit sans bruit : `explorer.inference-poll-timeout-ms` était livré dans `application.yml` avec une valeur, portait un getter et un setter, n'était lu par rien, et ne figurait dans aucun tableau. Un bouton réglable, livré, silencieusement inerte, invisible du seul contrôle qui existe pour tenir les réglages honnêtes. Les **lectures de champ** comptent autant que les appels de getter : `consumer-group-prefix` et `internal-topic-prefix` sont appliqués depuis le `@PostConstruct` d'`ExplorerConfig`, qui touche le champ sans passer par l'accesseur — un contrôle qui ne chercherait que `getX()` les déclarerait morts tous les deux. Ce qu'il ne fait **pas**, c'est de l'atteignabilité : `explorer.max-concurrent-jobs` a un lecteur qu'aucun chemin HTTP ne peut atteindre aujourd'hui, et il passe — savoir qu'un chemin est inatteignable est un outil bien plus gros, et un grep qui prétendrait y répondre serait pire que celui qui dit ce qu'il couvre. **Un sous-arbre peut être lié par une classe à lui**, et c'est ce que l'arrivée du serveur MCP a appris au script : `explorer.mcp.*` est lié par `McpProperties`, une classe `@ConfigurationProperties(prefix = "explorer.mcp")`, pas par un getter d'`ExplorerConfig` — le test getter-ou-champ déclarait donc tout le sous-arbre mort et se trompait sur chacune de ses clés. Une clé est désormais lue aussi quand une classe déclare ce préfixe **et** que quelque chose l'injecte ; la seconde moitié compte, puisqu'une classe de propriétés que personne n'injecte est exactement le bouton mort que ce contrôle traque, un cran plus bas.
- **`docs/check-agent-scenarios.py` résout les scénarios de l'agent MCP contre le monde qu'ils décrivent** (même job `docs-links`). Un scénario `src/test/resources/eval/agent/*.yaml` fait deux affirmations sur le monde, et les deux pourrissent en silence. `fixture.requires` prétend que le cluster semé porte tel topic et telle clé — c'est exactement l'argument de `check-eval-fixture.py`, un répertoire plus loin : une fixture qui a dérivé du jeu de données qu'elle nomme n'échoue pas, elle évalue la mauvaise chose, avec assurance ; un scénario qui interroge `ORD-107` après que le seeder a cessé de le produire tourne encore, appelle encore un modèle, et rend encore un verdict — sur rien. Et `trace.mustCall` / `mustNotCall` nomment des outils MCP, résolus contre les déclarations `@McpTool(name = "…")` : un outil renommé laisse un `mustCall` que personne ne satisfait, ce qui se lit « l'agent ne l'a jamais appelé » — un scénario rouge qui accuse un modèle d'un renommage. **`mustNotCall` pourrit dans l'autre sens et c'est le pire des deux** : un outil interdit qui n'existe plus ne peut plus jamais être appelé, donc l'assertion passe pour toujours en ayant cessé d'asserter. Une troisième passe porte sur le scénario comme ensemble : `expectRefusal` et `serverConfig` doivent s'accorder, faute de quoi le scénario attend une garde que rien n'a armée et échoue sur « la garde n'a pas tiré » — une phrase vraie à propos d'un scénario qui n'a jamais pu la faire tirer. Un répertoire vide n'est **pas** un échec : un harnais en construction n'a légitimement pas encore de scénario, et un contrôle qui rougit sur un répertoire vide est un contrôle qu'on supprime au lieu de le satisfaire.
