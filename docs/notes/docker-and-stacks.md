...
- **The app writes under `/app` and both paths are volumes**: `logs/` (`logging.file.name`) and `data/` (`explorer.settings-store-path`, `explorer.flink-table-store-path` — the Settings page's own input and the hand-written `CREATE TABLE`s were lost on every recreate; `explorer.flink-job-store-path` was the third and went with the Dashboard's job table). They are named volumes, never a bind-mounted *file*: `./Kafkaexplorer.log:/app/logs/kafkaexplorer.log` pointed at a path absent from the checkout, so Docker created a directory there and Logback could not open its log at all. That broken mount was also the stated reason the runtime image stayed root; both images now run as `USER 10001:10001`, owning `/app/logs` and `/app/data`, and a named volume inherits that ownership where a host bind mount would not.
...
- **La release ouvre la PR qui rattrape le dépôt : le pin de la stack hub, et le changelog.** `compose/spectra-hub.yml` nomme l'image de l'Explorer à la main (`${EXPLORER_IMAGE_TAG:-…}`), Dependabot ne sait pas lire cette forme, et le pin devient périmé à l'instant précis où le job `docker` publie — le seul endroit du dépôt qui sache qu'une version vient d'exister. Laissé à la main, ce n'est pas la release qui s'en aperçoit : c'est **la pull request suivante qui touche un fichier surveillé par `hub-changes`** et qui échoue sur `check-image-pins.py --published`, à propos de quelque chose qu'elle n'a pas changé — arrivé deux fois en une journée, sur deux branches sans rapport. L'étape ouvre donc une **pull request**, jamais un push sur `main` : toutes les entrées de `main` sont des merge commits relus, et cette PR touche les fichiers de la stack, donc `hub-changes` déclenche `spectra-hub-stack` et la nouvelle valeur est vérifiée **contre le registre** avant d'être fusionnée — le mécanisme qui signale le problème est celui qui valide la correction. Trois garde-fous : elle ne s'exécute que sur un tag **stable** (même règle que `latest` — une pré-version ne déplace pas le pin de la stack que les gens exécutent), elle est **best-effort** (`continue-on-error` : l'image est déjà publiée, une release ne doit pas être déclarée en échec parce qu'un suivi n'a pas pu s'ouvrir), et elle est écrite avec `git` et `gh` plutôt qu'une action tierce — c'est le chemin de release, qui porte déjà `contents: write` et un login registre. Elle a besoin de `pull-requests: write` sur le job et du réglage « Allow GitHub Actions to create and approve pull requests » ; s'il manque, la branche est poussée et un `::warning::` nomme le réglage.

  **Le changelog est fermé sur la même branche, et c'est le même défaut vu d'un autre côté.** Rien ne
  renommait `## [Unreleased]` au moment d'une release et aucun contrôle ne lit ce fichier, donc les
  entrées s'y empilaient en affirmant qu'elles n'étaient pas sorties — **trente versions** ainsi,
  `v1.7.1` à `v1.9.11`, le dernier titre de version du fichier huit semaines derrière le dernier tag.
  La différence avec le pin est instructive : un pin périmé finit par faire échouer la pull request
  suivante, un changelog périmé n'échoue nulle part, et c'est celui qui n'avait pas de garde-fou qui a
  duré le plus longtemps. Une seule branche porte les deux, les deux devenant faux au même instant et
  pour la même raison. Trois précautions propres à cette moitié : elle est **idempotente** (une section
  `[X.Y.Z]` déjà présente n'est pas refilée, donc une relance ne duplique rien — le bump du pin l'est
  désormais aussi, un pin qui nomme déjà la version cherchée n'étant plus confondu avec un pin
  introuvable, ce qui faisait mourir une release rejouée sur « has it been renamed? ») ; une release qui n'a
  rien inscrit reçoit quand même un titre renvoyant à ses notes, une version absente se lisant comme
  une version perdue ; et son échec **ne peut pas emporter le bump du pin** — le pin décide de l'image
  que les gens exécutent, donc le script du changelog tourne dans son propre processus, la PR s'ouvre
  avec ce qui a marché, et un `::error::` final dit ce qui n'a pas marché plutôt que de laisser une
  étape verte le sous-entendre. Les liens de comparaison sont dérivés du fichier lui-même (la référence
  `[Unreleased]:` nomme déjà le tag précédent) et non de `git describe`, qui demanderait l'historique
  des tags que ce clone superficiel ne récupère volontairement pas.
