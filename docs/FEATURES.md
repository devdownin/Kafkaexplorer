...
- **Multi-Provider Support**: Compatible with **Claude (Anthropic)**, **Open Source models** (via OpenAI-compatible APIs like Ollama), and **SpectraLLM** (self-hosted private RAG/fine-tuned models — `docker-compose-spectra-hub.yml` runs the pair from published images, nothing built locally, with overlays for a GPU, a smaller model, memory limits, or having SpectraLLM index the topics themselves). See the [LLM provider guide](LLM-PROVIDERS.md).
