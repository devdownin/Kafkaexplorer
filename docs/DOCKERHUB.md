...
| `EXPLORER_MCP_ALLOWED_TOPIC_PREFIXES` | `*` | Restrict every agent tool to these topic prefixes at once, e.g. `demo.,sandbox.`. Checked before any Kafka call — a scope check that runs after the read has already disclosed what it refused. `EXPLORER_MCP_ALLOWED_GROUP_PREFIXES` does the same for consumer groups. |
