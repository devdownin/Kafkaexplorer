...
1. [Périmètre étudié](#1-périmètre-étudié)
2. [Matrice comparative fonctionnelle](#2-matrice-comparative-fonctionnelle)
...
Neuf implémentations recensées, regroupées en quatre familles.

| # | Serveur | Famille | Langage | Licence | Maturité |
|---|---------|---------|---------|---------|----------|
| 1 | **KIP-1318** (`tools/mcp-server`, Apache Kafka) | First-party | Java | Apache 2.0 | *Under Discussion* — non livré |
| 2 | **confluentinc/mcp-confluent** | Vendeur OSS | TypeScript/Node 22+ | MIT | GA, 50+ tools |
| 3 | **Confluent Managed MCP** (global + regional) | Vendeur SaaS | — | propriétaire | GA, majoritairement read-only |
| 4 | **tuannvm/kafka-mcp-server** | Communauté | Go (franz-go + mcp-go) | MIT | v2.x, actif |
| 5 | **Redpanda — `rpk cloud mcp`** | Vendeur | Go | BSL/propriétaire | beta |
| 6 | **Redpanda — Agentic Data Plane / managed Kafka** | Vendeur SaaS | — | propriétaire | GA, **produce uniquement** |
| 7 | **Redpanda — Remote MCP** (tools en Redpanda Connect YAML) | Vendeur SaaS | YAML | propriétaire | GA |
| 8 | **GCP Managed Service for Apache Kafka MCP** | Cloud | — | propriétaire | GA |
| 9 | **kanapuli / Joel-hanson / aswinayyolath / jonyx225** | Communauté | Go, Python | MIT/Apache | mono-mainteneur, CRUD |

Les serveurs 9 sont fonctionnellement équivalents entre eux (CRUD topics + produce/consume + groupes) ; ils sont traités comme un bloc « baseline communautaire ».

---

## 2. Matrice comparative fonctionnelle

Légende : ● complet · ◐ partiel / sous conditions · ○ absent · ⧗ planifié

### 2.1 Plan de contrôle Kafka

| Capacité | KIP-1318 | mcp-confluent | Confluent Managed | tuannvm | Redpanda rpk | Redpanda ADP | GCP | Baseline |
|---|---|---|---|---|---|---|---|---|
| Lister / décrire topics | ● | ● | ● | ● | ● | ○ | ● | ● |
| Créer / supprimer topic | ● | ● | ○ | ○ | ● | ○ | ● | ● |
| Altérer configs topic | ● | ● | ◐ (lecture) | ◐ (describe) | ● | ○ | ● | ○ |
| `delete_records` / partitions | ● | ◐ | ○ | ○ | ◐ | ○ | ○ | ○ |
| Groupes : lister / décrire | ● | ● | ◐ | ● | ● | ○ | ● | ◐ |
| Lag **en offsets** | ● | ● | ● (métriques) | ● | ● | ○ | ● | ○ |
| Lag **en temps** (âge du plus vieux non lu) | ○ | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| Reset offsets / retrait de membres | ● | ◐ | ○ | ○ | ● | ○ | ○ | ○ |
| **ACLs** | ● | ◐ | ○ | ○ | ● | ○ | ○ | ○ |
| Quorum KRaft / feature versions | ● | ○ | ○ | ◐ | ◐ | ○ | ○ | ○ |
| Share groups (KIP-932) / Streams groups | ● | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| Transactions (abort, fence) | ● | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| Kafka Connect | ● | ● | ● + diagnostic IA | ○ | ◐ | ○ | ● | ○ |
| Schema Registry | ⧗ | ● | ● (lecture) | ○ | ◐ | ○ | ○ | ○ |

### 2.2 Plan de données

| Capacité | KIP-1318 | mcp-confluent | Confluent Managed | tuannvm | Redpanda ADP | Baseline |
|---|---|---|---|---|---|---|
| `produce` simple | ● | ● | ○ | ● | ● | ● |
| `produce` batch | ● | ◐ | ○ | ○ | ○ | ○ |
| `produce` transactionnel (EOS) | ● | ○ | ○ | ○ | ○ | ○ |
| `consume` borné | ● (cap 100 / 1 Mo) | ● | ● (échantillon) | ● | ○ | ◐ (non borné) |
| Consommation sans rebalance (`assign()`) | ● | ◐ | — | ◐ | — | ○ |
| Désérialisation Avro/Protobuf | ⧗ | ● | ● | ○ | ○ | ○ |
| **SQL sur topics** | ○ | ◐ (Flink **Confluent Cloud** uniquement) | ○ | ○ | ◐ (Connect YAML) | ○ |

### 2.3 Analyse — la zone vide

| Capacité | Tous les serveurs existants |
|---|---|
| Inférence de schéma (JSON/XML/Avro) sur topic non typé | ○ |
| Traçage d'une clé **à travers** N topics (corrélation cross-topic) | ○ |
| Latence par hop / détection du goulot d'un pipeline | ○ |
| Modèle entité-relation déduit entre topics | ○ |
| Analyse DLQ/retry avec **taux d'échec** rapporté au topic source | ○ |
| Audit cluster scoré (poison messages, doublons, drop-off de flux) | ◐ tuannvm : health-check générique |
| Enveloppe de couverture explicite sur les résultats partiels | ○ |
| Distinction « mesure impossible » vs « valeur nulle » | ○ |

### 2.4 Sécurité & gouvernance

| Contrôle | KIP-1318 | mcp-confluent | Confluent Managed | tuannvm | Redpanda rpk | Baseline |
|---|---|---|---|---|---|---|
| Mode read-only global | ● `mcp.readonly` | ◐ via block-list | ● par conception | ○ | ◐ `--allow-delete` off | ○ |
| Allow/deny-list d'outils | ● | ● (+ fichiers) | ○ | ○ | ○ | ○ |
| Scoping par préfixe de topic/groupe | ● | ○ | ○ | ○ | ○ | ○ |
| Taint guard (donnée lue → outil mutant) | ● best-effort | ○ | ○ | ○ | ○ | ○ |
| Approval token hors bande sur destructif | ● | ○ | ○ | ○ | ○ | ○ |
| Dry-run / blast radius | ● | ○ | ○ | ○ | ○ | ○ |
| DLP / masquage PII & secrets en sortie | ● | ○ | ○ | ○ | ○ | ○ |
| Contrôle d'exfiltration sur `produce` | ● | ○ | ○ | ○ | ○ | ○ |
| Piste d'audit inviolable (topic append-only) | ● | ○ | ● (RBAC/logs) | ○ | ● | ○ |
| Hook policy engine externe (OPA/Cedar) | ● | ○ | ○ | ○ | ○ | ○ |
| Rate limiting + circuit breakers par dépendance | ● | ○ | ● | ○ | ○ | ○ |
| OAuth 2.1 sur HTTP | ● | ● | ● | ● | ● | ○ |
| Codes JSON-RPC dédiés (-32040…-32047) | ● | ○ | ○ | ○ | ○ | ○ |
| **Console de supervision de l'usage MCP** | ○ | ○ | ◐ (logs cloud) | ○ | ◐ | ○ |

### 2.5 Exploitation

| | KIP-1318 | mcp-confluent | tuannvm | Baseline |
|---|---|---|---|---|
| Transport stdio | ● | ● | ● | ● |
| Transport HTTP streamable | ● (stateless) | ● (+SSE) | ● | ○ |
| Primitive **Resources** | ● 20 URIs `kafka://` | ◐ | ● 4 | ○ |
| Primitive **Prompts** | ⧗ phase 2+ | ○ | ● 4 diagnostics | ○ |
| Multi-cluster | ⧗ | ● | ○ | ○ |
| Métriques serveur (JMX/OTLP) | ● | ◐ | ○ | ○ |
| **Découverte de la surface par un humain** | CLI `--list-tools` | CLI `--list-tools` | README | README |
