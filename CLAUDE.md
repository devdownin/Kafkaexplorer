...
- `FlinkSqlService` — executes SQL against Kafka topics using embedded Flink `LocalEnvironment`; per-request table registration ensures isolation
  - **SELECT execution**: when `explorer.flink-select-enabled` is true (default), SELECT runs through the real Flink planner via `executeViaFlinkPlanner()`, falling back to `kafkaDirectSelect()` on any planner failure. A process-lifetime circuit breaker (`FLINK_SELECT_FAILURE_THRESHOLD` = 3) stops attempting the planner after repeated failures, so a still-present `FlinkRelMetadataQuery` NPE does not add a failed planner attempt to every query. Historically this NPE forced all SELECTs through the direct reader (reproduced on Flink 1.18/1.20/2.0); the Flink 2.3 migration re-enabled the planner path with this fallback. `CREATE TABLE` / `EXPLAIN` always go through Flink. Engine used is reported in `QueryResult.engine()` (`FLINK` vs `KAFKA_DIRECT`).
