...
- `FlinkJobStore` — **what the Flink engine has been asked to run, kept across a restart** (`data/flink-jobs.json`), mirrored from `FlinkSqlService`'s in-memory `heldJobs` by `syncPersistedJobs()`. It is what `GET /api/query/jobs` serves and what the Dashboard's "Flink SQL Jobs" panel lists (`getActiveJobs()` is `listActive()`, i.e. the store filtered on `isTerminal`). `FLINK-JOBS-AUDIT.md` is the review; five decisions in it are load-bearing and none is visible from the shape of the code.
  - **Three states of the world used to be written down as one.** "The job ended", "the runtime no longer holds it" and "we could not read its status" are different facts, and the third is not an observation at all. `buildJobSummary`'s generic catch filed a status poll that ran out of its 150 ms as `UNKNOWN` — which the store counts as **terminal** — so one slow answer stamped an `endedAt` on a running job, dropped it out of the Dashboard's list and put it on the retention clock, where a long `INSERT INTO` could be pruned out of the store while it ran. `FlinkSqlService.STATUS_UNAVAILABLE` is that third state and is **not** terminal; the `IllegalStateException` branch above it, which really does mean the job is over, is unchanged — and `isTerminal` is package-visible precisely so nothing re-decides what terminal means. The in-memory half was always right (`JobInfo.endedAt` stays null there), which is why a test pinned it and the defect lived in the store.
  - **A cancellation aimed at a job that already ended records the attempt, never the verdict.** The no-live-`JobClient` branch of `cancelQuery` wrote `UNKNOWN` over the record, so pressing **Kill** on a dashboard card whose job finished between two five-second polls destroyed the only account of how it finished. The status is left alone when the stored job is terminal (`null` status means "keep what is there"), and the request becomes a history entry under the status that was actually observed.
  - **A synchronous read hands its job back when the request returns** (`releaseSyncJob`, from a `finally` on `executeViaFlinkPlanner`). It used to stay in `heldJobs` until some *other* caller ran the sweep — which on a headless deployment is nobody, while `MetricService` adds one every 30 s for any metric the planner answers, each holding a `JobClient`. Same shape as the `getHeldJobs()` defect one method over, hidden the same way: an open browser is a sweeper. The cancel endpoint still finds the entry *while* the query runs, which is the whole point of a client-supplied query id.
  - **Bounded in count as well as in time, and the count is the bound that binds.** `explorer.flink-job-retention-hours` is right for what this store was written for — Job-mode submissions, a handful a day — and wrong for what feeds it: every statement the planner runs as a job, one per Run in the editor and ~2 900 a day per planner-answered metric. `MAX_RETAINED_JOBS` (200) drops the oldest **ended** jobs first and never a running one, which is the row the Dashboard, the 409 repoint guard and the lineage graph are all asking about. Retention also **reaches the file** now: `prune()` says whether it removed anything and the readers flush when it did, `load()` included, or a store nothing writes to again keeps its expired records for good.
  ...
  - **The record is on screen, not only in the file** (`components/dashboard/FlinkJobCard.tsx`, reading logic in `pages/flinkJobHistory.ts`). `FlinkJobSummary` drops `statusDetail`, `errorMessage` and the dated history, so the Dashboard's card showed a red `FAILED` badge whose reason existed only in `data/` — and `GET /api/query/jobs/{queryId}`, which serves all three, was called by nothing. The card opens onto them **on the click and once**: the Dashboard polls every five seconds by default, so a read per card per turn would make everyone pay for a detail nobody is looking at, and folding then reopening must not re-ask. A read that fails leaves the server's reason under the card rather than a toast — it is what you came to read. The pure module keeps the distinction the store was fixed for (`UNAVAILABLE` is described as a status that could not be read, never as an ending) and refuses to invent: an entry whose timestamp is unusable is dropped rather than dated to 1970, and a negative duration answers `null` rather than 0, which would assert instantaneity. The card was extracted from `Dashboard.tsx` in the same change — it had outgrown the loop that rendered it.
  - **The live registry is `heldJobs`, not `activeJobs`** (`getHeldJobs()`, not `getActiveJobsDetails()`). "Active" meant two different things on two methods a caller has to choose between: `getActiveJobs()` answers from the *store*, filtered on `isTerminal`, while this map is the `JobClient`s the runtime holds right now. A suffix is not a distinction. Its two populations have two lifetimes and the field says so: a Job-mode submission stays for as long as it runs, a synchronous read only for the duration of its HTTP request — which it must, since that is how `POST /api/query/cancel/{queryId}` finds it.
  - **Credentials are masked at the door, and the file is replaced rather than written in place.** A Flink `CREATE TABLE` embeds Kafka client properties, and the case that made it concrete is the one that looks harmless: `submitJob` **rejects** a non-INSERT statement and files it first, so a hand-written DDL pasted into the editor in Job mode put its SSL passwords and its `sasl.jaas.config` secret into `data/` and into `GET /api/query/jobs`. `create()` goes through `DdlGeneratorService.maskSensitiveProperties`; nothing here replays the SQL, unlike `FlinkTableStore`, so it costs nothing. And it reads and writes through **`JsonStoreFile`** — the class next door written for exactly this, whose javadoc says "two stores need the same thing" while there were three — so a crash mid-write no longer costs every job record instead of the one being written. `update()` also compares substance before writing: its caller is a five-second poll and a running job answers `RUNNING` every time, so the whole file was being rewritten to record a fact about the clock. **Both bounds were argued before they were measured, and the measurement is in `FLINK-JOBS-AUDIT.md`**: a write serialises the whole list, so it costs 111 µs at 10 records and **17.3 ms at 10 000** (a 5.3 MB file) — and four planner-answered metrics on the default 30 s loop reach ~11 500 records inside the 24 h retention window. The count bound puts the ceiling at 436 µs. The poll's own bookkeeping is not the problem and never was: the status fan-out costs ~8 µs per job against the up-to-150 ms of serialised waiting it avoids, so a fifty-job poll spends under half a millisecond outside the status calls.
...
- `QueryController` — `/api/query/**`. A path-less `@PostMapping` duplicating `/run-sync` was removed: nothing here ever posted to it, so it was a second public entry point to the query engine that no test exercised — two paths to one behaviour is how they drift. `ddl-preview` reports a failure through `SqlErrorClassifier.explain()`, **not** `e.getMessage()`: that is null for a `NullPointerException` and `Map.of` rejects a null value, so the handler's own error path answered 500 in exactly the case where the caller most needs a reason. Its `@RequestParam` is named explicitly so the mapping resolves without `-parameters` on the compiler command line (the offline harness compiles with plain `javac`). `init` **reports why it came back empty** (`kafkaError` / `flinkError` on `QueryInitResponse`) instead of ending both of its probes in an empty catch: an unreachable broker, a wrong bootstrap address, an authorisation failure and a Flink runtime still starting up all rendered as the same "Engine offline · 0 tables · 0 topics". The probes stay independent — one side failing must not empty the other — and a broker that answers `pingDetail()` but refuses the metadata call keeps `health: true` with the reason beside it, since "offline" would send an operator to check a connection that works. `cancel` **says what it achieved** (`{cancelled, outcome}` from `FlinkSqlService.CancelOutcome`) rather than returning `void` behind a 200: a `KAFKA_DIRECT` scan has no Flink job by construction, so "nothing to cancel" is the common case and the UI must not report it as a cancellation — it says "Request aborted — the server finishes its in-flight read" instead. Still 200 either way, since nothing to cancel is a legitimate outcome of a well-formed request. `QueryControllerTest` pins the three endpoints with standalone MockMvc. `submitJob` **answers with a reason** (`ApiError`, the shape `MetricController.save` already serves) instead of an empty status body. It had two ways of saying nothing: the whitelist refusal travelled in a `ResponseStatusException`, whose reason lands in the `message` field of Spring's default error body — suppressed, since `server.error.include-message` is `never` — and every other `RuntimeException` was not caught at all. The browser reads `message` then `error` and had neither, so `describeApiError` fell back to the status label and the one gesture of the SQL editor with no fallback engine was also the only one that could not be diagnosed. The status splits on `SqlErrorClassifier` rather than on a second rule written here: a user error is a 400, an engine failure stays a 500, and both carry the engine's own sentence. Flink's wording for a missing INSERT *target* (`Cannot find table '…' in any of the catalogs`) was added to that classifier — a SELECT's source says `Object 'x' not found` and was already covered, so a typo in a sink name was the one name error answering 500.
  ...
  **What the removed Job mode left behind is gone too** (the `SubmittedJobPanel` component under `components/query/`, deleted with its test). That panel followed a submitted job — it re-read `GET /api/query/jobs/{queryId}` every 3 s for a minute, so a job that died on its first row did not stay green — and it was reachable from exactly one place: a submission. With submission removed, nothing rendered it, and the page still carried its whole apparatus: five pieces of state, a polling effect, a Stop handler, and a `GET /api/query/sink-ddl` call that offered to create the INSERT's target. All of it referenced an `executionMode` that no longer existed, so **the SPA had not type-checked since**: twelve `tsc` errors in this one file, which failed `npm run build`, which failed `mvn verify`, on `main`. A helper nobody calls is the affordance this codebase keeps removing; this was the same thing one size up — a whole feature's wiring left without the feature. What survives is the rule the two mode guards existed to serve: an `INSERT` is refused in the browser, **with its cause** ("INSERT INTO is not run by the SQL editor"), rather than being sent to the engine to be answered by the whitelist. `GET /api/query/sink-ddl` and `FlinkSqlService.submitJob` are left standing and are named below — both are covered by live tests, so removing them is a separate decision from making the build pass.
...
The version is anthropic-java 2.57.0, and it is read from `pom.xml` rather than restated here from
memory: this heading carried `2.16.1` for a long while, which is what `check-config-table.py`'s
`VERSION_CLAIMS` pass now exists to catch.
