...
- **Build every form and every table header out of the shared controls.** `PasswordInput` adds a
  reveal toggle and `autoComplete="new-password"`, because a secret typed blind fails at connection
  time with nothing to diagnose; wrap the fields in a real `<form onSubmit>` so Enter submits from
  any of them, which is why `Button` defaults to `type="button"` and a submit button says so
  explicitly; and a sortable column is `SortButton`, which carries the arrow saying what sorts and
  which way plus a 24 px target. A hand-rolled `<input className="…">` drifts from the tokens and
  skips the accessibility wiring, and a hand-rolled header drops the direction and the target size
  — measured when `SortButton` left `Dashboard.tsx`: that page fell from 5 undersized targets to 1
...
## Claude API Java SDK (anthropic-java 2.61.0)
