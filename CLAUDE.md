...
**Tests** — Vitest + `@testing-library/react` on jsdom (`src/test/setup.ts`): `navigation.test.ts`, `components/ui/ui.test.tsx`, `forms.test.tsx`, `ConfirmDialog.test.tsx`, `ScrollList.test.tsx`, `useVirtualRows.test.ts`, `pages/queryError.test.ts`, `pages/sqlScope.test.ts`, `components/topic/topicSearch.test.ts`. Run with `npm test`.
