...
- `AuditController` — `@RestController` under `/api/audit` only: `start` (409 + in-flight id when one is running), `status/{id}` (404 on an unknown id), `{id}/cancel` (202 asked / 409 already finished / 404 unknown), `last` (204 when no run yet). Do **not** add a `GET /audit` mapping — `/audit` is a client-side route and a controller mapping on it shadows `SpaController`, producing a circular-view-path 500 on a page refresh (there is no template engine).
