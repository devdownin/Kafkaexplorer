...
| W8 | The unreachable containers W7 left: the SQL editor's toolbar strip under `lg`, and whatever `--detail` still names once the pages are given time to settle | 1 | W1, W6 | **closed** — depended on W1; the probe keeps measuring it, so a real case would resurface. That last clause was true of *pages* and false of everything that opens: two truncation defects were later reported by hand, in a confirmation card and a suggestion list, on surfaces no URL brings up. The probe now opens four states (`STATES` in `layout-probe.mjs`) and gates them like pages — verified on the build that carried the defect, where `sql-editor·confirm` reports `unreachable=5` against 1 at rest and names the overflowing paragraph. |
